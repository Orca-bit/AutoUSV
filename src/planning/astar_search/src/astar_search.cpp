  ...
  const nav_msgs::msg::OccupancyGrid & costmap, const IndexXYT & index, const size_t theta_size)
{
  ...
  using xT = std::decay_t<decltype(pose_local.position.x)>;
  using yT = std::decay_t<decltype(pose_local.position.y)>;
  pose_local.position.x = static_cast<xT>(index.x) * static_cast<xT>(costmap.info.resolution);
  pose_local.position.y = static_cast<yT>(index.y) * static_cast<yT>(costmap.info.resolution);
...
AstarSearch::AstarSearch(const AstarParam & astar_param) : astar_param_(astar_param)
{
  transition_table_ = createTransitionTable(
    ...
    astar_param_.turning_radius_size,
    astar_param_.theta_size,
...
void AstarSearch::setOccupancyGrid(const nav_msgs::msg::OccupancyGrid & costmap)
{
  ...
  for (size_t i = 0; i < height; i++) {
    for (size_t j = 0; j < width; j++) {
      const auto cost = costmap_.data[i * width + j];
...
bool AstarSearch::setStartNode()
{
  ...
  start_node->theta =
    2.0 * M_PI / static_cast<double>(astar_param_.theta_size) * static_cast<double>(index.theta);
  start_node->gc = 0;
...
SearchStatus AstarSearch::search()
{
  ...
  while (!openlist_.empty()) {
    ...
    const auto msec = std::chrono::duration_cast<std::chrono::milliseconds>((now - begin)).count();
    if (msec > static_cast<std::decay_t<decltype(msec)>>(astar_param_.time_limit)) {
...
{
  auto check = [this](const auto & pose) -> bool {
    const auto pose_local = global2local(costmap_, pose);
    const auto index = pose2index(costmap_, pose_local, astar_param_.theta_size);
    return detectCollision(index);
  };
  // return true if any pose in trajectory has collision
  return std::any_of(trajectory.poses.cbegin(), trajectory.poses.cend(), check);
}
...
bool AstarSearch::isGoal(const AstarNode & node) const
{
  ...
  if (
    std::fabs(relative_pose.position.x) > astar_param_.goal_longitudinal_tolerance ||
