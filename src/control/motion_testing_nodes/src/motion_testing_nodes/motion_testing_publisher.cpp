...
: Node(node_name, rclcpp::NodeOptions{rcl_get_default_allocator()}),
  m_tf_pub{create_publisher<TFMessage>(tf_topic, rclcpp::QoS{10}.transient_local(), PubAllocT{})},
  m_state_pub{create_publisher<State>(state_topic, rclcpp::QoS{10}.transient_local(), PubAllocT{})},
  m_traj_pub{
    create_publisher<Trajectory>(traj_topic, rclcpp::QoS{10}.transient_local(), PubAllocT{})},
  m_timer{nullptr}
{
  if (profiles.empty()) {
...
void MotionTestingPublisher::match(
  const std::size_t tf_subs, const std::size_t state_subs, const std::size_t traj_subs) const
{
  ...
  // WARNING Dumb hack: publish trajectory early
  {
    ...
    tf.transforms[0].header.stamp = time_utils::to_message(
      time_utils::from_message(tf.transforms[0].header.stamp) - std::chrono::milliseconds(2LL));
    m_tf_pub->publish(tf);
...
void MotionTestingPublisher::add_state_and_tf(
  const Header & header, const TrajectoryProfile & prof, const TrajectoryPoint & pt) noexcept
{
  // Make transforms
