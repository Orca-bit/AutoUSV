...
static_assert(ACADO_NYN == 6, "Unexpected number of terminal reference variables");
...
void MpcController::zero_terminal_weights() noexcept
{
  ...
  acadoVariables.WN[(IDYN_VEL_TRAN * NYN) + IDYN_VEL_TRAN] = AcadoReal{};
  acadoVariables.WN[(IDYN_VEL_ANGULAR * NYN) + IDYN_VEL_ANGULAR] = AcadoReal{};
}
...
{
  static_assert(ACADO_NYN == 6, "Unexpected number of terminal reference variables");
  acadoVariables.WN[(IDYN_X * NYN) + IDYN_X] = static_cast<AcadoReal>(cfg.pose_x());
  acadoVariables.WN[(IDYN_Y * NYN) + IDYN_Y] = static_cast<AcadoReal>(cfg.pose_y());
  acadoVariables.WN[(IDYN_HEADING * NYN) + IDYN_HEADING] = static_cast<AcadoReal>(cfg.heading());
  acadoVariables.WN[(IDYN_VEL_LONG * NYN) + IDYN_VEL_LONG] =
    static_cast<AcadoReal>(cfg.longitudinal_velocity());
  acadoVariables.WN[(IDYN_VEL_TRAN * NYN) + IDYN_VEL_TRAN] =
    static_cast<AcadoReal>(cfg.lateral_velocity());
  acadoVariables.WN[(IDYN_VEL_ANGULAR * NYN) + IDYN_VEL_ANGULAR] =
...
void MpcController::apply_nominal_weights(const StateWeight & cfg, const Index start, Index end)
{
  ...
  static_assert(ACADO_WEIGHTING_MATRICES_TYPE == 2, "Weighting matrices should vary per time step");
  static_assert(ACADO_NY == 8, "Unexpected number of reference variables");
  for (Index i = start; i < end; ++i) {
    constexpr auto NY2 = NY * NY;
    const auto idx = i * NY2;
    acadoVariables.W[idx + (IDY_X * NY) + IDY_X] = static_cast<AcadoReal>(cfg.pose_x());
    acadoVariables.W[idx + (IDY_Y * NY) + IDY_Y] = static_cast<AcadoReal>(cfg.pose_y());
    ...
    acadoVariables.W[idx + (IDY_VEL_LONG * NY) + IDY_VEL_LONG] =
      static_cast<AcadoReal>(cfg.longitudinal_velocity());
    acadoVariables.W[idx + (IDY_VEL_TRAN * NY) + IDY_VEL_TRAN] =
      static_cast<AcadoReal>(cfg.lateral_velocity());
...
void MpcController::advance_problem(const Index count)
{
  ...
  // TODO error checking on count
  // TODO x[0] should be x0... double check logic
  (void)std::copy(
    &acadoVariables.x[NX * (count + 1U)],
    &acadoVariables.x[(HORIZON + 1U) * NX],
    &acadoVariables.x[NX]);
  (void)std::copy(
    &acadoVariables.y[NY * count], &acadoVariables.y[HORIZON * NY], &acadoVariables.y[0U]);
  (void)std::copy(
    &acadoVariables.u[NU * count], &acadoVariables.u[HORIZON * NU], &acadoVariables.u[0U]);
}
