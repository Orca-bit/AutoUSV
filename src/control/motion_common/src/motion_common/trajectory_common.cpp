...
{
  return usv::common::helper_functions::wrap_angle(target_yaw - base_yaw);
}
...
size_t findNearestIndex(const Points & points, const geometry_msgs::msg::Point & point)
{
  ...
  for (size_t i = 0; i < points.size(); ++i) {
    const auto dist = usv::common::geometry::distance_2d<float64_t>(points.at(i), point);
    if (dist < min_dist) {
      min_dist = dist;
      min_idx = i;
    }
  ...
  const float64_t max_yaw)
{
  ...
  for (size_t i = 0; i < points.size(); ++i) {
    const auto dist = usv::common::geometry::distance_2d<float64_t>(points.at(i), pose.position);
    if (dist > max_dist) {
      continue;
    }
  ...
  const Points & points, const size_t seg_idx, const geometry_msgs::msg::Point & p_target)
{
  ...
  const Vector3f target_vec{
    static_cast<float64_t>(p_target.x) - x_front,
    static_cast<float64_t>(p_target.y) - y_front,
    0.0};
...
float64_t calcSignedArcLength(const Points & points, const size_t src_idx, const size_t dst_idx)
{
  ...
  for (size_t i = src_idx; i < dst_idx; ++i) {
    dist_sum += usv::common::geometry::distance_2d<float64_t>(points.at(i), points.at(i + 1));
  }
  return dist_sum;
}
