...
Point ControllerBase::predict(const Point & point, std::chrono::nanoseconds dt) noexcept
{
  ...
  using usv::common::state_vector::variable::XY_ACCELERATION;
  using usv::common::state_vector::variable::XY_VELOCITY;
  ...
  // Get result
  auto ret = point;  // to fill the gaps in the model
  {
    ret.time_from_start =
...
void compute_diagnostic(
  const ControllerBase & ctrl, const State & state, bool use_temporal_reference, Diagnostic & out)
{
  Index ref_idx = {};
  try {
