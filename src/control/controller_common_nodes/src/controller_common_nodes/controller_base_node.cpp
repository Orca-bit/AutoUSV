  ...
  const std::string & diagnostic_topic)
{
  ...
  m_state_sub = create_subscription<State>(
    state_topic, QoS{10}, [this](const State::SharedPtr msg) { on_state(msg); }, SubAllocT{});
  ...
  m_tf_sub = create_subscription<TFMessage>(
    tf_topic, QoS{10}, [this](const TFMessage::SharedPtr msg) { on_tf(msg); }, SubAllocT{});
...
bool ControllerBaseNode::try_compute(const State & state)
{
  ...
  // these transform functions need specific target frame firstly, and then source frame
  ...
  auto state_tf = state;
  // do transform specific source frame first, and then target frame
  ...
  const auto diagnostic_fn = [this, start, &state, &diag]() -> void {
    if (m_diagnostic_pub) {
      ...
      compute_diagnostic(
        *m_controller, state, m_controller->get_base_config().is_temporal_reference(), diag);
