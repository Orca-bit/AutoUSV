...
TEST(ConstantTrajectory, Stationary)
{
  ...
  for (auto i = 0U; i < traj.points.size(); ++i) {
    ...
    EXPECT_DOUBLE_EQ(t.pose.orientation.w, static_cast<double>(1.0F));
    EXPECT_DOUBLE_EQ(t.pose.orientation.x, static_cast<double>(0.0F));
    EXPECT_DOUBLE_EQ(t.pose.orientation.y, static_cast<double>(0.0F));
    EXPECT_DOUBLE_EQ(t.pose.orientation.z, static_cast<double>(0.0F));
