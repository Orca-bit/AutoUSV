...
TEST_F(SanityCheck, Basic)
{
  ...
  const auto ctrl = std::make_shared<NmpcControllerNode>(
    "mpc_ctrl_test_node",
    "",
    cmd_topic,
    state_topic,
  ...
  for (const auto & cmd : cmds) {
    ...
    EXPECT_LT(fabsf(cmd.left_cmd - 0.0F), TOL) << idx;
    EXPECT_LT(fabsf(cmd.right_cmd - 0.0F), TOL) << idx;  // real
    ++idx;
  }
}
