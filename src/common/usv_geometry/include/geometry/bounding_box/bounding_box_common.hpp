...
#include <usv_msgs/msg/detected_object.hpp>
#include <usv_msgs/msg/shape.hpp>
...
///            appropriate point adapter defined
template<typename IT>
void compute_height(const IT begin, const IT end, usv_msgs::msg::Shape & shape)
{
  float32_t max_z = -std::numeric_limits<float32_t>::max();
  float32_t min_z = std::numeric_limits<float32_t>::max();
  for (auto it = begin; it != end; ++it) {
...
usv_msgs::msg::Shape GEOMETRY_PUBLIC make_shape(const BoundingBox & box);
...
usv_msgs::msg::DetectedObject GEOMETRY_PUBLIC make_detected_object(
