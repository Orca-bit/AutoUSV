...
using usv_msgs::msg::BoundingBox;
using usv::common::geometry::point_adapter::x_;
using usv::common::geometry::point_adapter::y_;
using usv::common::geometry::point_adapter::xr_;
using usv::common::geometry::point_adapter::yr_;
...
protected:
  ...
  void minimum_area_bounding_box()
  {
    ...
    box = usv::common::geometry::bounding_box::minimum_area_bounding_box(points);
  ...
  {
    // apex_test_tools::memory_test::start();
    box = usv::common::geometry::bounding_box::minimum_perimeter_bounding_box(points);
    // apex_test_tools::memory_test::stop();
  }
  ...
  {
    // apex_test_tools::memory_test::start();
    box = usv::common::geometry::bounding_box::eigenbox_2d(begin, end);
    // apex_test_tools::memory_test::stop();
  }
  template<typename IT>
  ...
  {
    // apex_test_tools::memory_test::start();
    box = usv::common::geometry::bounding_box::lfit_bounding_box_2d(begin, end);
    // apex_test_tools::memory_test::stop();
  }
...
using PointTypesBoundingBox =
  ::testing::Types<geometry_msgs::msg::Point32, usv::common::types::PointXYZIF>;
...
{
  using usv::common::geometry::closest_segment_point_2d;
  using usv::common::geometry::point_line_segment_distance_2d;
  // normal case
...
{
  using usv::common::geometry::closest_line_point_2d;
  // normal case
  TypeParam p = this->make(-1.0F, 0.0F);
  TypeParam q = this->make(-1.0F, 2.0F);
  TypeParam r = this->make(1.0F, 1.0F);
...
TYPED_TEST(BoxTest, Hull)
{
  ...
  for (uint32_t idx = 0U; idx < FUZZ_SIZE; ++idx) {
    const float th = ((idx * usv::common::types::TAU) / FUZZ_SIZE) + dth;
...
{
  using usv::common::geometry::bounding_box::details::Covariance2d;
