...
struct TestPoint
{
  usv::common::types::float32_t x;
  usv::common::types::float32_t y;
};
...
{
  const auto end_it = usv::common::geometry::convex_hull(points);
...
TEST_P(IntersectionTest, Basic) {
  ...
  const auto result =
    usv::common::geometry::convex_polygon_intersection2d(polygon1, polygon2);
...
TEST(IoUTest, PentagonRectangleIntersection) {
  ...
  const auto expected_intersection_area =
    usv::common::geometry::area_2d(intersection.begin(), intersection.end());
  ASSERT_FLOAT_EQ(expected_intersection_area, 1.5F);  // Pen & paper proof.

  const auto expected_union_area =
    usv::common::geometry::area_2d(polygon1.begin(), polygon1.end()) +
    usv::common::geometry::area_2d(polygon2.begin(), polygon2.end()) -
    expected_intersection_area;
  ASSERT_FLOAT_EQ(expected_union_area, (11.0F + 4.0F - 1.5F));  // Pen & paper proof.

  const auto computed_iou =
    usv::common::geometry::convex_intersection_over_union_2d(polygon1, polygon2);
...
TEST(IoUTest, NoIntersection) {
  ...
  EXPECT_FLOAT_EQ(
    usv::common::geometry::convex_intersection_over_union_2d(polygon1, polygon2), 0.0F);
}
