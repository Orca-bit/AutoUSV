...
#include <usv_msgs/msg/point_clusters.hpp>
...
using PointTypes = ::testing::Types<geometry_msgs::msg::Point32,
    usv::common::types::PointXYZIF>;

// Wrapper function for stubbing output of
// usv::common::geometry::check_point_position_to_line_2d
template<typename T>
int point_position_checker(const T & p1, const T & p2, const T & q)
{
  auto result = usv::common::geometry::check_point_position_to_line_2d(p1, p2, q);
  if (result > 0.0F) {
    return 1;
  } else if (result < 0.0F) {
    return -1;
...
// usv::common::geometry::check_point_position_to_line_2d and input_output vector for
...
// Templated struct defining the function parameters for
// usv::common::geometry::is_point_inside_polygon_2d and input_output vector for
// assertion of return values from the function
template<typename T>
...
TYPED_TEST_P(InsidePolygon, InsidePolygonTest) {
  for (size_t i = 0; i < InsidePolygon<TypeParam>::input_output.size(); ++i) {
    ...
    EXPECT_EQ(
      usv::common::geometry::is_point_inside_polygon_2d(
        input.polygon.begin(),
...
TEST(ordered_check, basic) {
  // CW
  std::vector<usv_msgs::msg::PointXYZIF> points_list = {
    make_points<usv_msgs::msg::PointXYZIF>(8.0, 4.0),
    make_points<usv_msgs::msg::PointXYZIF>(9.0, 1.0),
    make_points<usv_msgs::msg::PointXYZIF>(3.0, 2.0),
    make_points<usv_msgs::msg::PointXYZIF>(2.0, 5.0)
  };
  EXPECT_TRUE(usv::common::geometry::all_ordered(points_list.begin(), points_list.end()));
  ...
  EXPECT_FALSE(usv::common::geometry::all_ordered(points_list.begin(), points_list.end()));

  // Unordered
  points_list = {
    make_points<usv_msgs::msg::PointXYZIF>(0.0, 0.0),
    make_points<usv_msgs::msg::PointXYZIF>(1.0, 1.0),
    make_points<usv_msgs::msg::PointXYZIF>(1.0, 0.0),
    make_points<usv_msgs::msg::PointXYZIF>(2.0, 1.0)
  };
  EXPECT_FALSE(usv::common::geometry::all_ordered(points_list.begin(), points_list.end()));

  // Colinearity
  points_list = {
    make_points<usv_msgs::msg::PointXYZIF>(2.0, 2.0),
    make_points<usv_msgs::msg::PointXYZIF>(4.0, 4.0),
    make_points<usv_msgs::msg::PointXYZIF>(6.0, 6.0)
  };
  EXPECT_TRUE(usv::common::geometry::all_ordered(points_list.begin(), points_list.end()));
}
