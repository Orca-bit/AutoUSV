...
protected:
  ...
  using Real = decltype(usv::common::geometry::point_adapter::x_(std::declval<Point>()));

  auto area()
  {
    return usv::common::geometry::area_checked_2d(data_.begin(), data_.end());
  }

  void add_point(Real x, Real y)
  {
    namespace pa = usv::common::geometry::point_adapter;
    Point p{};
    pa::xr_(p) = x;
    pa::yr_(p) = y;
