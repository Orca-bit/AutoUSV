...
bool all_are_arithmetic()
{
  ...
  return usv::common::type_traits::conjunction<std::is_arithmetic<Ts>...>::value;
}
...
TEST(TestCommonTypeTraits, Index) {
  ...
  EXPECT_EQ(0, (usv::common::type_traits::index<std::int32_t, T>::value));
  EXPECT_EQ(1, (usv::common::type_traits::index<float64_t, T>::value));
}
...
TEST(TestCommonTypeTraits, Visit) {
  ...
  std::int32_t counter{};
  usv::common::type_traits::visit(t, [&counter](const auto &) {counter++;});
  EXPECT_EQ(2, counter);
  float64_t sum{};
  usv::common::type_traits::visit(
...
TEST(TestCommonTypeTraits, HasType) {
  ...
  struct T3 {};
  EXPECT_TRUE(
    (usv::common::type_traits::has_type<T1, std::tuple<T1, T2>>::value));
  EXPECT_FALSE(
    (usv::common::type_traits::has_type<T3, std::tuple<T1, T2>>::value));
  EXPECT_FALSE(
    (usv::common::type_traits::has_type<T1, std::tuple<>>::value));
}
...
TEST(TestCommonTypeTraits, TypeIntersection) {
  ...
  EXPECT_TRUE(
    ...
    usv::common::type_traits::intersect<A, B>::type>::value));
  EXPECT_TRUE(
    (std::is_same<A, usv::common::type_traits::intersect<A, A>::type>::value));
  EXPECT_TRUE(
    (std::is_same<std::tuple<>,
    usv::common::type_traits::intersect<A, C>::type>::value));
}
